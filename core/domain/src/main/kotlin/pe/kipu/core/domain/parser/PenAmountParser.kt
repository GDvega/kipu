...
/**
 * Extracts PEN amounts from receipt OCR text (e.g. S/ 10.50, S/0.10, S/ 1,234.50).
 */
object PenAmountParser {
    private val AMOUNT_PATTERN = Regex(
        """(?i)S/\s*([0-9]{1,3}(?:,[0-9]{3})*|[0-9]+)(?:\.([0-9]{2}))?""",
    )

    fun parse(text: String): Money? {
        val match = AMOUNT_PATTERN.find(text) ?: return null
        val wholePart = match.groupValues[1].replace(",", "")
        val fractionalPart = match.groupValues.getOrNull(2)?.takeIf { it.isNotEmpty() } ?: "00"
