...
class CalculateCategoryWeeklySpentUseCaseTest {

    private val useCase = CalculateCategoryWeeklySpentUseCase()
    ...
    fun `sums only confirmed expenses for category in week range`() {
        ...
        val total = useCase(CategoryIds.FOOD, movements, weekRange)
    ...
    fun `returns zero when no matching movements`() {
        val total = useCase(CategoryIds.FOOD, emptyList(), weekRange)
