...
class CalculateWeeklyEnvelopeTotalsUseCaseTest {

    private val useCase = CalculateWeeklyEnvelopeTotalsUseCase()
    ...
    fun `uses global remaining instead of summing per envelope remainings`() {
        ...
        assertNull(totals.weeklyDeficit)
    }
    ...
    fun `returns weekly deficit when global spending exceeds total limit`() {
        ...
        assertEquals(Money.of(BigDecimal("50.00")).getOrError(), totals.weeklyDeficit)
    }
